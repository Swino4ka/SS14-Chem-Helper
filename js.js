// Чтобы добавить новый материал, просто допишите новую запись
const materials = {
    ...
    "Ацетон": { "Нефть": 0.5, "Сварочное Топливо": 0.5, "Кислород": 0.5 },
    "Бензол": { "Водород": 1, "Углерод": 1 },
    "Бикаридин": { "Инапровалин": 0.5, "Углерод": 0.5 },
    "Бритвиум": { "Ласеринол": 1, "Бикаридин": 1 },
    ...
    "Дифенгидрамин": { "Диэтиламин": 0.33333, "Сварочное топливо": 0.33333, "Соль": 0.33333, "Углерод": 0.33333, "Этанол": 0.33333 },
    ...
    "Мутотоксин": { "Уран": 0.5, "Вестин": 1, "КосмическийКлей": 1 },
    "Некросол": { "Кровь": 1.5, "Омнизин": 0.5, "Криоксадон": 1 },
    "Натрий": { "Соль": 2 },
    "Нестабильный Мутаген": { "Радий": 0.33333, "Фосфор": 0.33333, "Хлор": 0.33333 },
    "Нефть": { "Сварочное Топливо": 0.33333, "Водород": 0.33333, "Углерод": 0.33333 },
    ...
    "СолевойРаствор": { "Вода": 0.8, "Соль": 0.2 },
    ...
    "Ферросилицит": { "Железо": 0.5, "Кремний": 0.5 },
...
function updateProductionList() {
  ...
  productionItems.forEach((item, index) => {
    ...
    li.textContent = `${item.quantity} юнит(ов) ${item.name}`;
...
function updateReagentsList() {
  ...
  productionItems.forEach(item => {
    ...
    for (let reagent in composition) {
      ...
      if (reagentsTotal[reagent]) {
        reagentsTotal[reagent] += amount;
      } else {
        reagentsTotal[reagent] = amount;
      }
    }
  ...
  for (let reagent in reagentsTotal) {
    const li = document.createElement('li');
    li.textContent = `${reagentsTotal[reagent]} ${reagent}`;
